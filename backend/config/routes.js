...
module.exports = app => {
	...
	app.route('/validateUser').all(app.config.passport.authenticate()).all(app.api.user.getByToken)
	...
	app.route('/articles').all(app.config.passport.authenticate()).get(admin(app.api.article.get)).post(admin(app.api.article.save))
